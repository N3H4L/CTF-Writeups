...
We are given a zip file that contains the Logs of a windows system in `.evtx` files.
...
Now, at the very first, I need to convert the `evtx` files into a readable format.

I am going to use a tool in python called https://github.com/williballenthin/python-evtx 

You can simply download the tool using pip.

```bash
$ pip install python-evtx
```

After installing, you have a script called `evtx_dump.py` that converts an `evtx` file into a XML file.

```bash
$ evtx_dump.py <input file>.evtx > <output file>.xml
```

Now you can just use a bash for-loop to iterate over the `evtx` files an save the corresponding XML file.

Now comes the main part.

If we talk about Windows log files, one of the most interesting files is the `Microsoft-Windows-PowerShell%4Operational.evtx` file.

Why so ? It stores PowerShell events, for example module logging is stored on this file. Also included in this `.evtx` file is ***Script block logging*** which will capture PowerShell code executed by an attacker.

```xml
Logs ❯ cat ../outputs/Microsoft-Windows-PowerShell%4Operational.evtx.xml 
...
The above is the contents of the log file.

Let me just highlight the most interesting part of the log file.
...
Well we can clearly see that the script is decoding the shellcode by XORing it with `0xd1`.

But apart from that, we can also see that the concatenated stage1 and stage2 is also being XORed with `0xd1`. 

Now this is odd. Let us try to XOR it and see what is there.
...
```python
Logs ❯ python3 ../script.py 
HTB{b3wh4r3_0f_th3_b}...3c1tsuj_f0_5t00
```

 

We can see half of the flag and the other half is inverted. Let us correct this.
...
So, that is all for this challenge. Hope you liked it. 🙂